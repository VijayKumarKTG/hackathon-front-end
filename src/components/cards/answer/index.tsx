...
const AnswerContentCard = (props: Props) => {
  ...
  const { write: post_comment, isLoading: isCommentPosting } = useContractWrite(
    {
      ...post_comment_config,
      onError(error) {
  ...
  const { write: vote_answer, isLoading: isAnswerVoteInProgress } =
    useContractWrite({
      ...vote_answer_config,
      onError(error) {
        setErrorTitle(error.message);
  ...
  }, [isAnswerVoteInProgress, isCommentPosting]);
  ...
  const { write: choose_best_answer, isLoading: isChoosingBestAnswer } =
    useContractWrite({
