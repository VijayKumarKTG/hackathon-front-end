...
const AskQuestion = () => {
   ...
   * Tags state
   */
  const [tagsState, setTagsState] = useState<Array<string>>([]);
  ...
  const handleInputKeyDown = (event: any) => {
    if (event.key === 'Enter' && inputValue.trim()) {
      if (inputValue.trim() && tagsState.length <= 7)
        setTagsState([...tagsState, inputValue.trim()]);
      setInputValue('');
    }
  };
  ...
  return (
    <div className='bg-darkblue py-14 px-2 md:px-20 xl:px-36'>
      <form
                ...
                {tagsState.map((tag, index) => (
                  <span
                    ...
                    <button
                      ...
                      <span className='text-[#929292] text-lg'>&#x2715;</span>
                    </button>
                    {tag}
                  </span>
                ))}
        ...
        <button
          type='submit'
          disabled={title === '' || question === '' || tagsState.length === 0}
